...
export function SprintRetroModal({
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[#071018]/86 px-4">
      <section
        className={`pointer-events-auto w-[min(640px,100%)] border-2 bg-black/84 p-6 text-center ${accentClass}`}
      >
        <div className={`text-sm font-bold uppercase tracking-wide ${eyebrowClass}`}>
          {title}
        </div>
        <div className="mt-2 text-3xl font-bold text-white">{message}</div>

        <div className="mt-6 border border-cyan-300/20 bg-[#071018]/80 p-4 text-left">
        ...
        <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
          <button
            type="button"
            className="cursor-pointer border-2 border-cyan-300/70 bg-cyan-500/15 px-5 py-3 font-bold text-cyan-100 transition hover:border-cyan-100 hover:bg-cyan-400/25"
            onClick={onRestart}
          >
            Start New Sprint
          </button>
          {isVictory ? (
            <button
              type="button"
              className="cursor-pointer border-2 border-amber-300/75 bg-amber-500/15 px-5 py-3 font-bold text-amber-100 transition hover:border-amber-100 hover:bg-amber-400/25"
...
function RetroStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-cyan-300/15 bg-black/45 p-3">
      <div className="text-xl font-bold text-white">{value}</div>
      <div className="mt-1 text-xs font-bold uppercase tracking-wide text-cyan-300">
        {label}
      </div>
    </div>
  );
